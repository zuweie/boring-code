 ...
 * @LastEditTime: 2026-07-26 14:19:19
...
{
    if (CG_NODE_TYPE(thiz) == e_operand && cg_list_size(thiz->_base.vertex.in) == 1) {
        return cg_list_get(thiz->_base.vertex.in, 0);
    } else if (CG_NODE_TYPE(thiz) != e_operand ) {
        CG_DEBUG("ERROR <%d@%s>: cg_operand_get_operator, thiz is not a operand\n", __LINE__, __FILE__);
    } else {
        CG_DEBUG("ERROR <%d@%s>: cg_operand has not one operator(acturlly is %d)\n", __LINE__, __FILE__, cg_list_size(thiz->_base.vertex.in));
    }
    return NULL;
}
#endif