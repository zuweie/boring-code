...
static int __marker_key_cmp(void* k1, void* k2) 
{
    return strcmp(k1, k2);
}
...
static int __deep_first_search(cg_vertex_t* p_start, cg_vertex_t* p_end, cg_hash_t* marker, cg_list_t* p_searching, cg_list_t* p_paths)
{
    cg_hash_set(marker, p_start->id, NULL);
    ...
    if (p_start == p_end) {
        ...
        while (p_first != CG_LIST_HEAD(p_start->out_vertexes))
        {
            ...
            if (!cg_hash_has(marker, p_vertex->id)) {
                __deep_first_search(p_vertex, p_end, p_searching, p_paths);
            }
...
{   
    ...
    cg_hash_t* p_marker    = cg_hash_create(__marker_hash, __marker_key_cmp)
