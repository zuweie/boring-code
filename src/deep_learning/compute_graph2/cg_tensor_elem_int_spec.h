 ...
 * @LastEditTime: 2026-05-16 15:38:58
...
#define cg_elem_ptr_opt(dest_ptr, opt1_ptr, opt2_ptr, opt) \
        *((int*)(dest_ptr)) = *((int*)(opt1_ptr)) opt *((int*)(opt2_ptr))

#define cg_elem_number_opt(dest_ptr, number1, number2, opt) \
        *((int*)(dest_ptr)) = (number1) opt (number2)

#define cg_elem_pn_opt(dest_ptr, opt_ptr, number, opt) \
        *((int*)(dest_ptr)) = *((int*)(opt_ptr)) opt (number)

#define  cg_elem_ptr_assign(dest_ptr, opt_ptr) \
