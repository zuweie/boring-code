 ...
 * @LastEditTime: 2026-05-04 14:20:43
...
static int __float_assign(cg_ref_t dest, cg_ref_t elem1, cg_ref_t elem2) 
{
    float* res  = (float*) dest;
    float* opt1 = (float*) elem1;
    *res = *opt1;
...
cg_elem_spec_t* cg_float_spec() 
{
    if (!spec) {
        ...
        cg_elem_spec_opt_register(spec, elem_opt_assign,   __float_assign);
    }
    return spec;
}