...
static inline znode2_t* layer_first(nn2_t* nn2) 
{
    return nn2->znode_head.next;
}

static inline znode2_t* layer_last(nn2_t* nn2) 
{
    return nn2->znode_head.prev;
}

static inline znode2_t* layer_head(nn2_t* nn2) {
    return &nn2->znode_head;
}

static inline znode2_t* layer_tail(nn2_t* nn2) {
    return &nn2->znode_head;
}
...
static inline int layer_remove(znode2_t* z) 
{
    z->prev->next = z->next;
    z->next->prev = z->prev;
    return 0;
