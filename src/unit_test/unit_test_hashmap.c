 ...
 * @LastEditTime: 2020-10-27 01:14:02
...
#define ENTITY_2_HASH_KEY(tv) do {       \
    HashNode * pnode  =  tp2(tv); /*It_getptr(it);*/   \
...
static void test_hashmap_set (void) 
{
    ...
    //CU_ASSERT(1);
    Hashmap_(hashmap, Hashmap_cleanup);
...
static void test_hashmap_del(void)
{
    ...
    //CU_ASSERT(1);
    CU_ASSERT_FALSE(Hashmap_has(hashmap, uiv(11)));
    CU_ASSERT_FALSE(Hashmap_has(hashmap, uiv(22)));
    CU_ASSERT_FALSE(Hashmap_has(hashmap, uiv(3)));
